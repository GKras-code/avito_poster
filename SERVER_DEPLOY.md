...
2. Backend открывает реальное окно браузера на той машине, где запущен backend.
3. Пользователь в этом окне сам нажимает `Вход`, вводит логин и пароль, решает captcha и вводит SMS-код.
4. После успешного входа backend автоматически сохраняет `storage_state`, `cookies` и профиль браузера.

Важно:

- текущая реализация не встраивает браузер прямо внутрь DOM веб-страницы;
- окно браузера появляется именно на хосте backend;
- на headless Ubuntu-сервере без графического `DISPLAY` этот сценарий не откроет окно;
- чтобы показывать браузерную сессию прямо через веб, понадобится отдельный VNC/noVNC слой.
